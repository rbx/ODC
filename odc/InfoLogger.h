...
class CInfoLogger
{
  ...
  private:
    class CBackend : public boost::log::sinks::basic_sink_backend<boost::log::sinks::combine_requirements<boost::log::sinks::synchronized_feeding>::type>
    {
  ...
  private:
    ...
    void log(ESeverity _severity, const std::string& _channel, const std::string& _msg)
    {
        ...
        InfoLoggerContext context(m_context, { { InfoLoggerContext::FieldName::Partition, partition }, { InfoLoggerContext::FieldName::Run, run } });
    ...
    int convertSeverityToLevel(ESeverity _severity)
    {
        switch (_severity) {
            case ESeverity::debug:
                return 14;
